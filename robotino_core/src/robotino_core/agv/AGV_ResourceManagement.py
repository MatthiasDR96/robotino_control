        ...
        while True:
            ...
            if self.agv.status == 'BUSY':
                if self.agv.battery_status < self.agv.battery_threshold:
                    ...
                    # charge_time = amount_to_charge / self.charging_factor
                    ...
                    if self.agv.routing_approach == 'dmas':
                        ...
                        start_time = self.agv.slots[-1][0] + self.agv.slots[-1][
                            1] if self.agv.task_executing else self.agv.env.now

                        # Get path and slots
                        self.agv.reserved_paths[charging_task.order_number], \
                        self.agv.reserved_slots[charging_task.order_number], _ = \
                            self.agv.routing.dmas(start_node, [charging_task.pos_A], start_time)
