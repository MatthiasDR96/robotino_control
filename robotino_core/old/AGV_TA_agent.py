...
class AGV_TA_agent:
	...
	def main(self):
		...
		while True:
			...
			# Get tasks to assign
			tasks_to_auction = self.comm_main.sql_select_tasks_to_auction(self.params['id'])
	...
	def handle_client(self, conn):
		...
		if task['message'] == 'announce':
			...
			# Get local task list
			local_task_list = comm.sql_select_local_task_list(self.params['id'])
	...
	def local_consensus(self):
		...
		while True:
			...
			# Get tasks to assign
			tasks_to_assign = self.comm_local_consensus.sql_select_local_task_list(self.params['id'])
	...
	def compute_bid(self, local_task_list, new_task, substract, comm):
		...
		# Get task executing
		task_executing = comm.sql_select_executing_task(robot['id'])
