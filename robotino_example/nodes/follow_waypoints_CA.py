...
def turnTowardsGoal(goal):

	print("Turn towards goal")
	alpha = 0.2
	while alpha > 0.1 or alpha < -0.1:
...
def compute_cartesian_from_laser():
...
def collisionAvoidanceController():
	...
	[points_in_left_roi, points_in_right_roi] = compute_cartesian_from_laser() 

	# Collsion avoidance
	if(points_in_right_roi > threshold and points_in_left_roi < threshold):
		print("Turn left")
		cmd = Twist()
		cmd.angular.z = 40 * math.pi / 180
		cmd_pub.publish(cmd)
	elif(points_in_left_roi > threshold and points_in_right_roi < threshold):
		print("Turn right")
		cmd = Twist()
		cmd.angular.z = - 40 * math.pi / 180
		cmd_pub.publish(cmd)
	elif(points_in_left_roi > threshold and points_in_right_roi > threshold):
		print("Turn till clear view")
		while points_in_right_roi > threshold or points_in_left_roi > threshold:
			[points_in_left_roi, points_in_right_roi] = compute_cartesian_from_laser()
			cmd = Twist()
			cmd.angular.z = - 40 * math.pi / 180
			cmd_pub.publish(cmd)
		...
		print("Continue path")
		cmd = Twist()
		[vu, omega] = calculateControlOutput(goal)
		cmd.linear.x = vu
		cmd.angular.z = omega
		cmd_pub.publish(cmd)
